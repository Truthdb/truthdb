...
use truthdb_core::relational::ResultColumn;
...
fn curcmd_of(command: truthdb_core::relational::DoneCommand) -> u16 {
    use truthdb_core::relational::DoneCommand;
    match command {
    ...
    async fn event<W: AsyncWrite + Unpin>(
        ...
        match event {
            ...
            BatchEvent::Error(error) => {
                ...
                if error.level >= truthdb_core::relational::FATAL_SEVERITY {
                    self.fatal = true;
                }
            }
...
mod render_tests {
    ...
    use truthdb_core::relational::{BatchOutcome, ResultColumn, RowSet, StatementResult};
            ...
            match result {
                StatementResult::Rows(rowset) => {
                    ...
                    tx.send(BatchEvent::StatementDone {
                        ...
                        command: truthdb_core::relational::DoneCommand::Select,
                    })
                    .unwrap();
                }
                StatementResult::RowsAffected(n) => tx
                    .send(BatchEvent::StatementDone {
                        count: Some(*n),
                        in_transaction: in_xact,
                        command: truthdb_core::relational::DoneCommand::Other,
                    })
                    .unwrap(),
